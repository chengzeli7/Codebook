...
- 👆 **生物识别** - 支持指纹/面部识别快速解锁
- 📋 **智能剪贴板** - 自动识别并保存账号密码（支持2秒延迟检测）
- 🚫 **防截屏保护** - 防止敏感信息被截屏/录屏
- 🎨 **iOS风格UI** - 简约苹果设计语言，专注核心功能
...
- 更换设备后，密码数据无法迁移（安全设计）
- 请牢记主密码，忘记后无法恢复
- Android 12+剪贴板限制：应用从后台返回前台后需等待2秒才能读取剪贴板
...
1. **Android 12+剪贴板限制**: 后台应用无法读取剪贴板，已实现2秒延迟检测方案
2. **生物识别误判**: 面部识别在弱光环境下识别率较低
3. **数据库加密**: 使用Room本地数据库 + 字段级AES-256-GCM加密
